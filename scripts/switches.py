        ...
        if "sys.upgrade" in sys_dict:
            if sys_dict["sys.upgrade"] == "true":
                if os.path.isfile(AUTO_UPGRADE_FILE):
                    ...
                    try:
                        xbmc.executebuiltin('XBMC.Notification("'+"Raspbmc updates are enabled"+'",,2000,"'+'")')
                    except:
                        syslog.syslog("Raspbmc updates are enabled")
                ...
                if not os.path.isfile(AUTO_UPGRADE_FILE):
                    ...
                    f.close()
                    try:
                        xbmc.executebuiltin('XBMC.Notification("'+"Raspbmc updates are disabled"+'",,2000,"'+'")')
                    except:
                        syslog.syslog("Raspbmc updates are disabled")
